...
const float L1 = 0.06f;// Valeur originale : 0.3f -> 0.01 premier test
const float L2 = 0.3f;// 0.3
const float L3 = 0.0f;//0.2
const float L4 = 0.2f;//0.2
const float L5 = 0.077f;//0.1
const float L6 = 0.014f;  //0.1
float theta[7];
float anc_theta[7];
vector position;
vector orientation;
...
{
  ROS_INFO("I heard: [%f ,%f, %f, %f, %f, %f, %f, %f]",msg->data[0],msg->data[1],msg->data[2],msg->data[3],msg->data[4],msg->data[5], msg->data[6], msg->data[7]);
  ...
  while(ros::ok())
  {
     ...
     {       
        ...
        if(i==2)
        {
            ...
            ROS_INFO("LA MODIF est elle PASSEE");
        }    
        else
        {
     ...
     theta_int[0]=(theta[0] + PI)*(4095/(2*PI));
     //ROS_INFO("J0ok");
     //jointState.position[1] = theta[1]*PI/180;
     theta_int[1]=(theta[1] + PI)*((3072-1024)/(2*PI)) +1024;//0-->6.28 => (3072-1024)/6.28 +1024
     //ROS_INFO("J1ok");
     //jointState.position[2] = theta[2]*PI/180;
     theta_int[2]=(theta[2] + PI)*((3072-1024)/(2*PI)) +1024;//0-->6.28 => (3072-1024)/6.28 +1024
     //ROS_INFO("J2ok");
     //jointState.position[3] = theta[3]*PI/180;
     theta_int[3]=(theta[3] + PI)*((3072-1024)/(2*PI)) +1024;//0-->6.28 => (3072-1024)/6.28 +1024
     //ROS_INFO("J3ok");
     //jointState.position[4] = theta[4]*PI/180;
     theta_int[4]=(theta[4] + PI)*((3072-1024)/(2*PI)) +1024;//0-->6.28 => (3072-1024)/6.28 +1024
     //ROS_INFO("J4ok");
     //jointState.position[5] = theta[5]*PI/180;
     theta_int[5]=(theta[5] + PI)*(1024)/(2*PI);
     //ROS_INFO("J5ok");; 
     //theta_int[6]=(theta[6] + PI)*(1024)/(2*PI);
     theta_int[6]=(gripreboot->x*512)+500; // Avancement griper 500=fermé
       printf("theta_int modifié");
     std_msgs::Int16MultiArray retour_angle;
 
     
     retour_angle.data={theta_int[0],theta_int[1],theta_int[2],theta_int[3],theta_int[4],theta_int[5],theta_int[6]};
