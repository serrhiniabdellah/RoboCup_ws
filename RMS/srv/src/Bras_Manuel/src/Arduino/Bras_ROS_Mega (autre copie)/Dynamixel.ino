...
// JE TENTE DE CHQNGER LE 1024 3072 en 0 $=)%
...
void WriteTest(){  
    ...
    SERIAL_SERVO.write(0xFF); // Start bytes
    SERIAL_SERVO.write(0xFF);
    SERIAL_SERVO.write(0x00);
    SERIAL_SERVO.write(0xFE); // Servo ID
    SERIAL_SERVO.write(0x09);
    SERIAL_SERVO.write(0x00); // Length of message (2 + 3 parameters)
    SERIAL_SERVO.write(0x82); // Write
    SERIAL_SERVO.write(0x00); // Adresse   
    SERIAL_SERVO.write(0x07);  
    SERIAL_SERVO.write(0x00);
    SERIAL_SERVO.write(0x02);  
    SERIAL_SERVO.write(0x00);   
    SERIAL_SERVO.write(0x01);   
    SERIAL_SERVO.write(0x02);   
    SERIAL_SERVO.write(0x03);   
    SERIAL_SERVO.write(0x04);   
    SERIAL_SERVO.write(0x05);   
    SERIAL_SERVO.write(0x06);
    SERIAL_SERVO.write(0x07);   
    byte notchecksum = ~lowByte(0xFE + 0x09 + 0x00 + 0x82 + 0x00 + 0x07 + 0x00 + 0x02+0x00+0x01+0x02+0x03+0x04+0x05+0x06+0x07);
    ...
    delay(SERIAL_DELAY);*/
     ...
     byte Length = 0x04;
    byte notchecksum = ~lowByte(0x01 + Length+0x02 + 0x2B+0x01); // avec vitesse
    digitalWrite(PIN_RTS,HIGH); // put MAX485 into transmitting mode
    delay(10);
    SERIAL_SERVO.write(0xFF); // Start bytes
    SERIAL_SERVO.write(0xFF);
    SERIAL_SERVO.write(0x01); // Servo ID
    SERIAL_SERVO.write(Length); // Length of m
    SERIAL_SERVO.write(0x02); // Servo ID
    SERIAL_SERVO.write(0x2B); // Reboot
     SERIAL_SERVO.write(0x01); // Reboot
    ...
    int monbuffer[65];
    for(int i=0;i<SERIAL_SERVO.available();i++)
    {
      monbuffer[i]=SERIAL_SERVO.read();
      Read.print(monbuffer[i]);
      Read.print(".");
    }
    Read.println();
